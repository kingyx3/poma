...
9. Waits for the VM startup script to write `/var/lib/poma/vm-ready` with the expected startup revision and current boot id, and verifies the `poma` user plus Docker and cron are ready. The deploy does not treat cloud-init's generic boot-finished marker as sufficient because the Google startup script can still be running. If startup fails before readiness, the script writes `/var/lib/poma/vm-startup-failed` so the workflow can print bootstrap diagnostics quickly instead of waiting for the full readiness window. If the VM is reachable but still bootstrapping at the readiness deadline, the workflow fails with diagnostics instead of resetting and restarting package work; it resets once only when SSH/IAP never becomes reachable.
10. Pauses the app cron schedule and stops any old compose-run app containers before upload, then uploads one trimmed deployment bundle (`.env`, `.dockerignore`, Dockerfile, compose file, Python package metadata/source, deploy script, and cron file) through IAP SSH, retrying short-lived `scp` attempts to survive transient IAP backend or SSH-key propagation failures.
11. Pulls the selected prebuilt image and runs a fixture-backed dry-run smoke test when `deploy_smoke=true`.
...
The VM startup script keeps boot light: it removes stale readiness/failure sentinels, stops cron while bootstrapping so old jobs do not compete with package work, creates the fixed `poma` app identity and runtime directories before package work, installs only missing host packages, skips `apt-get update/install` on reboots where Docker/cron/curl/python are already present, clears apt lists after host bootstrap, and writes `/var/lib/poma/vm-ready` only after Docker and cron are enabled and active. Ubuntu cloud images can already reserve UID/GID `1000` for the default image identity, so startup creates the `poma` user/group with the same numeric IDs when needed instead of failing on that expected base-image state. The app deploy still checks Docker Compose immediately before pulling the prebuilt image so a missing Compose plugin fails at the concrete install step instead of blocking VM readiness. IB Gateway, IBC, headless GUI support, and `ibgateway.service` are provisioned by the IB Gateway Ops workflow. Auto CI/CD runs Gateway Ops after its dev/stg deploy jobs when deploy or Gateway paths changed; manually dispatched deploys should be followed by an explicit IB Gateway Ops `restart`, `configure-paper`, or `configure-live` action as appropriate. This keeps cloud-init fast so the deploy's VM-readiness wait does not stall on the multi-minute IB Gateway install. Gateway Ops uploads helper scripts as one small archive, installs diagnostics before the heavy repair starts, and gives the known first-time IB Gateway installer path a separate 780-second cap; repeated runs use the runtime sentinel and should skip the heavy install. The deploy bundle includes the rendered `.env` so the workflow makes one IAP upload instead of separate package and environment transfers; each upload attempt is bounded at 4 minutes. Before upload, deploy removes the app crontab, stops cron, and asks Docker Compose to stop old one-shot app containers so the e2-micro is not pulling the new image while stale monitor/reconcile jobs are running. Before extracting, deploy removes stale package-owned files from older full-repo uploads while preserving `data`, `reports`, `state`, and `logs`; it then extracts the bundle as `poma`, which avoids recursive package chown work on the small VM. Auto CI/CD PR deploys skip the on-VM dry-run smoke, Docker pull diagnostics, and dangling-image prune; the runner has already built the exact image under test and CI has already exercised the app. Manual and release deploys keep `deploy_smoke=true` by default and still run the smoke path plus bounded Docker cleanup. The deploy package and `.dockerignore` exclude tests, docs, Git metadata, local state, reports, logs, and unrendered `.env` files so IAP uploads stay small and runtime secrets are not sent to the Docker daemon. The upload-and-install step has a 40-minute ceiling, and readiness is capped at 12 minutes so failures surface with diagnostics while still allowing a fresh e2-micro boot to finish Docker package setup. If the remote install step exits non-zero, the SSH command prints focused diagnostics for the `poma` identity, extracted package ownership, Docker/Compose availability, service state, disk usage, Docker cache usage, and the redacted compose image selection before returning the original failure status.

After Terraform apply, the workflow exports the stable non-sensitive outputs needed by deploy (`instance_name`, `zone`, and `startup_revision`) into job environment variables with bounded retries. The upload/install step consumes those exported values instead of opening Terraform state again, so transient state-backend failures fail early in the output-export step and do not interrupt VM upload/install after packaging has already started.
