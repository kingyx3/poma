...
| `IBKR_LOGIN_ID_PAPER` | Dev IB Gateway Ops `configure-paper` | Paper IBKR username passed to the VM only during explicit dev Gateway configure actions. All configure actions are disabled for `stg`. |
| `IBKR_LOGIN_SECRET_PAPER` | Dev IB Gateway Ops `configure-paper` | Paper IBKR password passed to the VM only during explicit dev Gateway configure actions. All configure actions are disabled for `stg`. |
| `IBKR_LOGIN_ID` | Prd IB Gateway Ops `configure-live` | Live IBKR username passed to the VM only during explicit production live Gateway configure actions. All configure actions are disabled for `stg`. |
| `IBKR_LOGIN_SECRET` | Prd IB Gateway Ops `configure-live` | Live IBKR password passed to the VM only during explicit production live Gateway configure actions. All configure actions are disabled for `stg`. |
...
IBKR broker login credentials intentionally live in GitHub Environment secrets. Account id secrets are account selectors; they are not broker login passwords. For dev paper Gateway validation, set `IBKR_LOGIN_ID_PAPER` / `IBKR_LOGIN_SECRET_PAPER` alongside `IBKR_ACCOUNT_PAPER`. For `stg`, Auto CI/CD deploys the VM/app and renders `IBKR_ACCOUNT_PAPER`, but does not run or allow `configure-paper` or `configure-live`.
...
5. Add `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`, and the account selector secret required by the selected trading mode (`IBKR_ACCOUNT_PAPER` for paper, `IBKR_ACCOUNT` for live). Add `IBKR_LOGIN_ID_PAPER` / `IBKR_LOGIN_SECRET_PAPER` only to `dev`, where `configure-paper` is allowed; add `IBKR_LOGIN_ID` / `IBKR_LOGIN_SECRET` only to `prd`, where `configure-live` is allowed.
