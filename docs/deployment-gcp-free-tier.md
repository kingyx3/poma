...
| `IBKR_LOGIN_ID_PAPER` | Dev IB Gateway Ops `configure-paper` | Paper IBKR username passed to the VM only during explicit dev Gateway configure actions. `configure-paper` is disabled for `stg`. |
| `IBKR_LOGIN_SECRET_PAPER` | Dev IB Gateway Ops `configure-paper` | Paper IBKR password passed to the VM only during explicit dev Gateway configure actions. `configure-paper` is disabled for `stg`. |
