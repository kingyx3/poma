...
The VM-side image pull is bounded at 8 minutes, and the deploy smoke test is bounded at 3 minutes. Local development can keep using docker-compose.yml with build enabled; the VM uses docker-compose.vm.yml with an image reference.
