...
| Every order `QuoteBlocked`: "missing quote timestamp" | Gateway is authenticated and trade-enabled but no market data subscription/entitlement reaches this account (common right after enabling paper market data sharing, before it fully propagates) | `poma ibkr-check` now fails Gateway configure with the underlying IBKR error text (e.g. "354: Requested market data is not subscribed.") when this happens, so **IB Gateway Ops** `configure-paper`/`configure-live` surfaces it directly instead of only discovering it later during a rebalance. Fix the account's market data entitlement, then re-run configure-paper/live. |
