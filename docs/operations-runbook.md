...
The next scheduled rebalance also checks the order ledger for anything still open from a *prior* session, or from a *different run* within the same session, before planning. By default (`STALE_ORDER_POLICY=block`) it blocks until those orders are reconciled or cancelled manually; set `STALE_ORDER_POLICY=cancel` to have it cancel them automatically instead. Orders still open from the exact *same run* are reported but never block a retry: if that retry resubmits with the same run_id, any orderRef already recorded non-terminally in the ledger is not resubmitted (an `IdempotentReplay` result is returned for it instead).
