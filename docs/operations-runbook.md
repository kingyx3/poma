...
| `BrokerUnavailable` / `Not connected` during paper | IB Gateway is listening but not API-ready/authenticated, the Gateway logged in without Trading/Market Data permissions, the Gateway's Read-Only API toggle is on, or the gateway connection dropped before IBKR accepted the order batch | Run **IB Gateway Ops** configure-paper, verify `poma ibkr-check`, then clear rebalance state via **IB Gateway Ops** only if you want cron to retry the same session. The what-if trading-permission preflight fails fast on Read-Only API (no orders submitted) rather than letting a few live orders through and get rejected first. |
