...
| `FRACTIONAL_ORDER_MODE` | yes | `cash_quantity` | How a fractional-sized trade quantity is expressed to IBKR. The IBKR API rejects fractional share quantities outright (error 10243 "Fractional-sized order cannot be placed via API"), so `cash_quantity` (default) submits fractional-sized trades as cash-quantity orders — the order carries the dollar amount and IBKR derives the fractional share count. Requires fractional share trading permission on the account (Client Portal → Settings → Account Settings → Trading Experience & Permissions); `poma ibkr-check` and the pre-submission readiness gate verify this with a cash-quantity what-if probe. Whole-share quantities are still submitted as plain share orders. `whole_shares` instead floors every trade to whole shares and drops trades below one share — safe without fractional permission, but a small managed cap cannot hold anything priced above its per-position notional. `share_quantity` submits fractional share quantities as-is (rejected by IBKR today; escape hatch only). |
| `NON_FRACTIONAL_TICKERS` | no | `` (empty) | Comma-separated tickers to round down to whole shares instead of sending fractional orders, regardless of `FRACTIONAL_ORDER_MODE`. Every other ticker defaults to fractional-friendly sizing, since a small managed cap depends on fractional quantities to hit target weights. Only list tickers confirmed to reject fractional orders at the broker. |
